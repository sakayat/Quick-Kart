...
import { addToCart, clearCart, decreaseCart, removeCart } from "../features/cartSlice";
	...
	return (
		<div className="container mx-auto cart">
			<h2 className="uppercase text-2xl font-bold text-center py-6">
				{data.length > 0 ? "your cart " : "your cart empty"}
			</h2>
            <div className="text-center">
                {data.length === 0 && <Link className="pb-5" to="/products">← Continue shopping</Link>}
            </div>
			{data.length > 0 && (
                <>
			...
			{data.map((product) => (
				<div
					...
					<div className="col-span-3 md:col-auto md:text-start text-center">
						<button onClick={() => decreaseCartItem(product)} className="py-2 px-4 bg-gray-300">-</button>
						<span className="py-2 px-4">{product.cartTotalQuantity}</span>
						<button onClick={() => increaseCartItem(product)} className="py-2 px-4 bg-gray-300">+</button>
					</div>
